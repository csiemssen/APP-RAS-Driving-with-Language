...
class TestModelEvaluation(unittest.TestCase):
  def test_intern_vl_eval(self):
    ...
    output_file = os.path.join(data_dir, "output", f"{sanitize_model_name(engine.model_path)}_output.json")
    submission_file = os.path.join(data_dir, "output", f"{sanitize_model_name(engine.model_path)}_submission.json")
    self.assertTrue(os.path.exists(output_file), "Output file should be created.")
    self.assertTrue(os.path.exists(submission_file), "Submission file should be created.")
